...
VideoWidget *VideoWidget::lastFocusWidget = 0;

VideoWidget::VideoWidget(LayoutPos pos, QWidget *parent)
    : QWidget(parent)
    , m_isPressed(false)
    , m_pressMoving(false)
    , m_isCheckable(false)
...
void VideoWidget::parseVideoArea(const LayoutInfo &info, QWidget *parentWidget, QRect area, QMap<LayoutPos, VideoWidget *> &cacheMap)
{
    const int column_count =  info.column_count;
    const int row_count = info.row_count;

    //用于后面隐藏播放控件
    QList<VideoWidget *>unused_video_widget_list = cacheMap.values();
    ...
    for(int row = 0; row < row_count; row ++) {
        for(int column = 0; column < column_count; column ++) {
            ...
            if(!cacheMap.contains(pos)) {
                cacheMap.insert(pos, new VideoWidget(pos, parentWidget));
            }
            VideoWidget *w = cacheMap.value(pos);
    ...
    for(int k = 0; k < info.cells.count(); k ++) {
        ...
        for(int p1 = 0; p1 < cell.column_spans; p1 ++) {
            for(int p2 = 0; p2 < cell.row_spans; p2 ++) {
                ...
                VideoWidget *w = cacheMap.value(pos, NULL);
                if(w) {
    ...
    QList<VideoWidget *>all_video_widget_list = cacheMap.values();
    for(int k = 0; k < all_video_widget_list.count(); k ++) {
        VideoWidget *w = all_video_widget_list[k];
...
void VideoWidget::updateImage(const FFmpegData &d)
{
    //轮播控制
...
void VideoWidget::toControlClose()
{
    removePlayer();
}

void VideoWidget::toControlFullScreen()
{
...
void VideoWidget::toControlFullScreenExit()
{
...
void VideoWidget::updatePlayListDevice()
{
...
void VideoWidget::dragEnterEvent(QDragEnterEvent *event)
{
    ...
    if (mimedata->hasFormat("VideoWidget/obid") && mimedata->data("VideoWidget/pos").toInt() != m_info.pos.value()) {
        event->accept();
        return;
    }
    //end
...
void VideoWidget::dropEvent(QDropEvent *event)
{
    ...
    if (event->mimeData()->hasFormat("VideoWidget/obid")) {
        QByteArray obid = event->mimeData()->data("VideoWidget/obid");
        this->addPlayer(obid);
        event->setDropAction(Qt::CopyAction);
        event->accept();
        return;
...
void VideoWidget::mouseMoveEvent(QMouseEvent *event)
{
    ...
    */
    ...
    if((event->pos() - m_pressPos).manhattanLength() < qApp->startDragDistance()
        ...
        m->setData("VideoWidget/obid", m_info.device_obid.toLocal8Bit());
        m->setData("VideoWidget/pos", QByteArray::number(m_info.pos.value()));
...
void VideoWidget::addPlayer(const QString &deviceObid)
{
...
void VideoWidget::addPlayerList(const QList<AlterPlayFrame> &playList)
{
    removePlayer();
...
void VideoWidget::runPlayerList()
{
    m_playListIndex = -1;
...
void VideoWidget::updateControlPanelGeometry()
{    
    m_controlPanel->setGeometry(lds::margin / 2, lds::margin / 2, this->width() - lds::margin, 50);
}

void VideoWidget::setCheckable(bool enabled)
{
    m_isCheckable = enabled;
}

void VideoWidget::setChecked(bool checked)
{
    m_isChecked = checked;
    update();
}

bool VideoWidget::isChecked()
{
    return m_isChecked;
}

void VideoWidget::setContextMenuDataList(QList<ContextMenuData> contextMenuDataList)
{
    m_contextMenuDataList = contextMenuDataList;
}

void VideoWidget::setGeometryX(int column_spans, int row_spans, QRect area, int column_count, int row_count)
{
...
QRect VideoWidget::rectX()
{
    return m_info.rect();
}

QString VideoWidget::device_obid()
{
    return m_info.device_obid;
}

const LayoutCell &VideoWidget::getInfo()
{
    return m_info;
}
